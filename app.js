...
function renderGrid() {
  grid.innerHTML = '';
  // La variable MAPS se define en cada archivo HTML antes de cargar este script
...
lb.canvas.style.webkitUserSelect = 'none';
lb.canvas.style.userSelect = 'none';
(function enableGestures() {
  const el = lb.canvas, pts = new Map(); let lastCenter = null, lastDist = 0;
  const getCenter = () => { const a = [...pts.values()]; return { x: (a[0].x + a[1].x) / 2, y: (a[0].y + a[1].y) / 2 }; };
  const getDist = () => { const a = [...pts.values()]; return Math.hypot(a[1].x - a[0].x, a[1].y - a[0].y); };
  if ('PointerEvent' in window) {
    el.addEventListener('pointerdown', e => { el.setPointerCapture(e.pointerId); pts.set(e.pointerId, { x: e.clientX, y: e.clientY }); if (pts.size === 1) { lb.dragging = true; lb.lx = e.clientX; lb.ly = e.clientY; el.style.cursor = 'grabbing'; } else if (pts.size === 2) { lastCenter = getCenter(); lastDist = getDist(); } });
    el.addEventListener('pointermove', e => { const p = pts.get(e.pointerId); if (!p) return; p.x = e.clientX; p.y = e.clientY; if (pts.size === 1 && lb.dragging) { const dx = e.clientX - lb.lx, dy = e.clientY - lb.ly; lb.tx += dx; lb.ty += dy; lb.lx = e.clientX; lb.ly = e.clientY; renderTransform(); } else if (pts.size >= 2) { const c = getCenter(), d = getDist(); lb.tx += (c.x - lastCenter.x); lb.ty += (c.y - lastCenter.y); lastCenter = c; const rect = el.getBoundingClientRect(); const factor = d / lastDist; lastDist = d; zoomAt(c.x - rect.left, c.y - rect.top, factor); } });
    ['pointerup', 'pointercancel', 'pointerleave', 'pointerout'].forEach(t => el.addEventListener(t, e => { pts.delete(e.pointerId); if (pts.size === 0) { lb.dragging = false; el.style.cursor = 'default'; } }));
  }
  el.addEventListener('dblclick', e => { const r = el.getBoundingClientRect(); zoomAt(e.clientX - r.left, e.clientY - r.top, 1.4); });
  let lastTap = 0;
  el.addEventListener('touchend', e => { const now = Date.now(); if (now - lastTap < 300 && e.changedTouches[0]) { const t = e.changedTouches[0]; const r = el.getBoundingClientRect(); zoomAt(t.clientX - r.left, t.clientY - r.top, 1.4); } lastTap = now; }, { passive: true });
})();
